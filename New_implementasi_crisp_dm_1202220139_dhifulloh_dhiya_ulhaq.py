...
| Kriteria Evaluasi | Indikator Keberhasilan (Threshold) |
|---|---|
| **Bayesian Information Criterion (BIC)** | Nilai terendah di antara model yang dibandingkan. Menunjukkan keseimbangan *goodness-of-fit* dan kesederhanaan. |
| **Entropy** | Nilai ≥ 0.80. Menunjukkan klasifikasi probabilitas yang tegas (minim *overlap* keraguan individu masuk klaster mana). |
| **Bootstrapped Likelihood Ratio Test (BLRT)** | *p-value* < 0.05. Secara statistik membuktikan model dengan *k* klaster signifikan lebih baik dari *k-1* klaster. |
| **Minimum Class Proportion** | Setiap profil/klaster harus mencakup minimal 5% dari total sampel agar tidak dianggap *spurious class* (anomali). |
| **Interpretasi Substantif** | Klaster memiliki perbedaan rata-rata (*centroid*) yang masuk akal secara teori psikologi klinis dan dapat di-label. |
...
# Upload file (support Colab dan local environment)
import io
try:
    ...
    # Jika tidak di Colab, baca dari path lokal
    import glob
    ...
    if csv_files:
        ...
        raise FileNotFoundError("Tidak ada file CSV ditemukan. Letakkan file dataset di direktori yang sama.")
df_raw.head(5)
...
# Memfilter model berdasarkan kriteria keberhasilan yang ditetapkan:
# 1. Entropy >= 0.80 (Klasifikasi tegas)
# 2. BLRT p-value < 0.05 (Signifikansi statistik k vs k-1)
# 3. Minimum Class Proportion >= 5% (Bukan spurious class)

valid_models = []

for r in results:
    # Hitung proporsi kelas terkecil untuk model k saat ini
    labels = r['Labels']
    counts = pd.Series(labels).value_counts(normalize=True)
    min_prop = counts.min()
    r['Min_Class_Prop'] = min_prop

    # Cek kriteria threshold
    if (r['Entropy'] >= 0.80 and
        r['BLRT_p'] < 0.05 and
        min_prop >= 0.05):
        valid_models.append(r)

# Fallback: jika tidak ada yang memenuhi kriteria ketat, cari yang mendekati
if not valid_models:
    valid_models = [r for r in results if r['Min_Class_Prop'] >= 0.05]
...
print("=" * 60)
print(f"  BIC (Lower is better) : {best['BIC']:,.1f}")
print(f"  Entropy (>= 0.80)     : {best['Entropy']:.4f}")
print(f"  BLRT p-value (< 0.05) : {best['BLRT_p']:.4f}")
print(f"  Min Class Prop (>= 5%): {best['Min_Class_Prop']*100:.2f}%")
print(f"  Silhouette Score      : {best['Silhouette']:.4f}")

print("\n📊 Justifikasi Berdasarkan Kriteria Keberhasilan:")
print(f"1. BIC: Nilai {best['BIC']:,.1f} adalah yang terendah, menunjukkan model paling efisien.")
print(f"2. Entropy: Skor {best['Entropy']:.4f} memenuhi threshold >= 0.80 (klasifikasi sangat tegas).")
print(f"3. BLRT: p-value {best['BLRT_p']:.4f} membuktikan model k={best_k} secara statistik lebih baik dari k={best_k-1}.")
print(f"4. Proporsi: Kelas terkecil mencakup {best['Min_Class_Prop']*100:.2f}% data (memenuhi syarat > 5%).")
print(f"5. Substantif: Model ini siap untuk di-label berdasarkan profil centroid di tahap selanjutnya.")

"""## Skenario k=1 sampai k=7
...
print("Memulai Iterasi Pembentukan Profil Laten (k=1 hingga k=7)...")
print("-" * 50)

for k in K_RANGE_2:
    # Eksekusi fungsi pemodelan untuk nilai k saat ini
    res = fit_and_evaluate_gmm(X_scaled, k)
    # Jalankan simulasi BLRT untuk membandingkan k dengan k-1 (k=1 default ke 1.0)
    blrt_p = simulate_blrt(X_scaled, k-1, k, bootstrap_draws=10) if k > 1 else 1.0
    res['BLRT_p'] = round(blrt_p, 4)
    # Simpan hasil iterasi
...
# Tampilkan kompilasi hasil iterasi
print("\n--- Tabel Kompilasi Hasil Iterasi GMM (k=1 hingga 7) ---")
df_results_2 = pd.DataFrame(results_2).drop(columns=['Model', 'Labels'])
...
# AIC Plot
axes2[0, 0].plot([r['k'] for r in results_2], [r['AIC'] for r in results_2],
             'o-', color='#3498db', linewidth=2, markersize=8)
axes2[0, 0].set_title('AIC per k', fontsize=14, fontweight='bold')
axes2[0, 0].set_xlabel('Jumlah Profil (k)')
axes2[0, 0].set_ylabel('AIC')
axes2[0, 0].set_xticks(list(K_RANGE_2))

# BIC Plot
axes2[0, 1].plot([r['k'] for r in results_2], [r['BIC'] for r in results_2],
             'o-', color='#e74c3c', linewidth=2, markersize=8)
axes2[0, 1].set_title('BIC per k', fontsize=14, fontweight='bold')
axes2[0, 1].set_xlabel('Jumlah Profil (k)')
axes2[0, 1].set_ylabel('BIC')
axes2[0, 1].set_xticks(list(K_RANGE_2))

# BLRT Plot
axes2[1, 0].plot([r['k'] for r in results_2], [r['BLRT_p'] for r in results_2],
             'o-', color='#f1c40f', linewidth=2, markersize=8)
axes2[1, 0].axhline(y=0.05, color='red', linestyle='--', alpha=0.5, label='Alpha (0.05)')
axes2[1, 0].set_title('BLRT P-Value per k', fontsize=14, fontweight='bold')
axes2[1, 0].set_xlabel('Jumlah Profil (k)')
axes2[1, 0].set_ylabel('P-Value')
axes2[1, 0].set_xticks(list(K_RANGE_2))
axes2[1, 0].legend()

# Entropy Plot
axes2[1, 1].plot([r['k'] for r in results_2], [r['Entropy'] for r in results_2],
             'o-', color='#27ae60', linewidth=2, markersize=8)
axes2[1, 1].axhline(y=0.80, color='red', linestyle='--', alpha=0.5, label='Threshold (0.80)')
axes2[1, 1].set_title('Entropy per k', fontsize=14, fontweight='bold')
axes2[1, 1].set_xlabel('Jumlah Profil (k)')
axes2[1, 1].set_ylabel('Entropy')
axes2[1, 1].set_xticks(list(K_RANGE_2))
axes2[1, 1].legend()

# Silhouette Plot
axes2[2, 0].plot([r['k'] for r in results_2], [r['Silhouette'] for r in results_2],
             'o-', color='#9b59b6', linewidth=2, markersize=8)
axes2[2, 0].axhline(y=0.50, color='red', linestyle='--', alpha=0.5, label='Threshold (0.50)')
axes2[2, 0].set_title('Silhouette per k', fontsize=14, fontweight='bold')
axes2[2, 0].set_xlabel('Jumlah Profil (k)')
axes2[2, 0].set_ylabel('Silhouette Score')
axes2[2, 0].set_xticks(list(K_RANGE_2))
axes2[2, 0].legend()

fig2.delaxes(axes2[2, 1])

plt.suptitle('Evaluasi Pemilihan Jumlah Profil Optimal (k=1 hingga 7)', fontsize=16, fontweight='bold', y=1.02)
plt.tight_layout()
plt.show()
...
for r in results_2:
    ...
    if (r['k'] > 1 and
        ...
        r['BLRT_p'] < 0.05 and
        min_prop >= 0.05):
        valid_models_2.append(r)

# Fallback: jika tidak ada yang memenuhi kriteria ketat
if not valid_models_2:
    valid_models_2 = [r for r in results_2 if r['k'] > 1 and r['Min_Class_Prop'] >= 0.05]

if not valid_models_2:
    valid_models_2 = [r for r in results_2 if r['k'] > 1]

# Pilih model dengan BIC terendah
best_2 = min(valid_models_2, key=lambda x: x['BIC'])
best_k_2 = best_2['k']
best_model_2 = best_2['Model']
best_labels_2 = best_2['Labels']

print("=" * 60)
print(f"🏆 MODEL TERBAIK TERPILIH (SKENARIO k=1-7): k = {best_k_2}")
print("=" * 60)
print(f"  BIC (Lower is better) : {best_2['BIC']:,.1f}")
print(f"  Entropy (>= 0.80)     : {best_2['Entropy']:.4f}")
print(f"  BLRT p-value (< 0.05) : {best_2['BLRT_p']:.4f}")
print(f"  Min Class Prop (>= 5%): {best_2['Min_Class_Prop']*100:.2f}%")
print(f"  Silhouette Score      : {best_2['Silhouette']:.4f}")

print("\n📊 Justifikasi Berdasarkan Kriteria Keberhasilan:")
print(f"1. BIC: Nilai {best_2['BIC']:,.1f} adalah yang terendah di antara kandidat valid, menunjukkan model paling efisien.")
print(f"2. Entropy: Skor {best_2['Entropy']:.4f} mengukur ketegasan probabilitas penugasan klaster.")
print(f"3. BLRT: p-value {best_2['BLRT_p']:.4f} membuktikan secara statistik bahwa model k={best_k_2} secara signifikan lebih baik daripada k={best_k_2-1}.")
print(f"4. Proporsi: Kelas terkecil mencakup {best_2['Min_Class_Prop']*100:.2f}% data (memenuhi syarat > 5% untuk menghindari spurious class).")
print(f"5. Substantif: Model ini siap untuk di-label berdasarkan profil centroid di tahap selanjutnya.")

"""## Skenario k=otomatis

## 4.2 Iterasi Model & Seleksi Model Terbaik
"""

# 1. Inisialisasi Model Otomatis dengan batas maksimal k=7 (sesuai Skenario 2)
bgm_auto_3 = BayesianGaussianMixture(
    n_components=7,
    covariance_type='full',
...
# 2. Latih model satu kali (algoritma otomatis memangkas klaster yang tidak perlu)
bgm_auto_3.fit(X_scaled)
labels_auto_3 = bgm_auto_3.predict(X_scaled)

# 3. Identifikasi berapa klaster yang benar-benar "aktif" (proporsi >= 5%)
weights_3 = bgm_auto_3.weights_
active_clusters_mask_3 = weights_3 >= 0.05
optimal_k_auto_3 = np.sum(active_clusters_mask_3)
min_prop_auto_3 = np.min(weights_3[active_clusters_mask_3])

# 4. Kalkulasi metrik untuk model otomatis
proba_auto_3 = bgm_auto_3.predict_proba(X_scaled)
safe_proba_auto_3 = np.clip(proba_auto_3, 1e-10, 1.0)

# Penyesuaian Entropy jika ternyata model memutuskan k=1
if optimal_k_auto_3 <= 1:
    entropy_auto_3 = 0.0
else:
    entropy_auto_3 = 1 - (-np.sum(safe_proba_auto_3 * np.log(safe_proba_auto_3))) / (len(X_scaled) * np.log(optimal_k_auto_3))

sil_score_auto_3 = silhouette_score(X_scaled, labels_auto_3) if len(np.unique(labels_auto_3)) > 1 else 0.0

# 5. Susun ke dalam format list valid_models_3 agar konsisten
best_3 = {
    'k': optimal_k_auto_3,
    'Entropy': round(entropy_auto_3, 4),
    'Silhouette': round(sil_score_auto_3, 4),
    'Min_Class_Prop': min_prop_auto_3,
    'Model': bgm_auto_3,
    'Labels': labels_auto_3,
    'Metode': 'Bayesian Dirichlet Process'
}

valid_models_3 = [best_3]

best_k_3 = best_3['k']
best_model_3 = best_3['Model']
best_labels_3 = best_3['Labels']

# 6. Tampilkan Hasil
print(f"🏆 MODEL TERBAIK TERPILIH (SKENARIO 3 - OTOMATIS): k = {best_k_3}")
print("-" * 60)
print(f"  Metode Seleksi        : {best_3['Metode']}")
print(f"  Entropy               : {best_3['Entropy']:.4f}")
print(f"  Min Class Prop (>= 5%): {best_3['Min_Class_Prop']*100:.2f}%")
print(f"  Silhouette Score      : {best_3['Silhouette']:.4f}")

print("\n📊 Justifikasi Pendekatan Otomatis (Bayesian GMM):")
print(f"1. Mekanisme: Model secara independen diberi batas atas 7 klaster, namun algoritma")
print(f"   Dirichlet Process secara otomatis mematikan klaster yang tidak relevan (bobot < 5%),")
print(f"   sehingga menyisakan {best_k_3} klaster aktif secara natural.")
print(f"2. Evaluasi: Tidak menggunakan metrik BIC/BLRT karena BGM mengoptimasi 'Variational Lower Bound'.")
print(f"3. Triangulasi: Jika hasil 'k' otomatis ini konvergen (sama) dengan hasil 'k' dari Skenario 1 & 2,")
print(f"   ini menjadi bukti matematis yang sangat kuat bahwa struktur profil data memang solid.")

# 1. Kumpulkan Kandidat Terbaik dari 3 Skenario
summary_data = []

# Skenario 1 (best dari k=2-5)
summary_data.append({
    'Skenario': 'Skenario 1 (k=2-5)',
    'k_Optimal': best_k,
    'BIC': round(best['BIC'], 1),
    'Entropy': best['Entropy'],
    'Min_Prop': round(best['Min_Class_Prop']*100, 2),
    'Model_Obj': best_model
})

# Skenario 2 (best dari k=1-7)
summary_data.append({
    'Skenario': 'Skenario 2 (k=1-7)',
    'k_Optimal': best_k_2,
    'BIC': round(best_2['BIC'], 1),
    'Entropy': best_2['Entropy'],
    'Min_Prop': round(best_2['Min_Class_Prop']*100, 2),
    'Model_Obj': best_model_2
})

# Skenario 3 (Bayesian Otomatis)
summary_data.append({
    'Skenario': 'Skenario 3 (Otomatis)',
    'k_Optimal': best_k_3,
    'BIC': 'N/A (Bayesian)',
    'Entropy': best_3['Entropy'],
    'Min_Prop': round(best_3['Min_Class_Prop']*100, 2),
    'Model_Obj': best_model_3
})

df_summary = pd.DataFrame(summary_data)

print("--- PERBANDINGAN HASIL LINTAS SKENARIO ---")
display(df_summary.drop(columns=['Model_Obj']))

# 2. Logika Penentuan Model Akhir (Final Decision)
# Kita prioritaskan Skenario 1/2 yang memiliki BIC terukur,
# namun jika Skenario 3 (Otomatis) menghasilkan k yang sama, itu memperkuat validitas.

# Mencari BIC terendah di antara model GMM (Skenario 1 & 2)
gmm_candidates = [s for s in summary_data if isinstance(s['BIC'], (int, float, np.float64))]
overall_best_entry = min(gmm_candidates, key=lambda x: x['BIC'])

final_best_model = overall_best_entry['Model_Obj']
final_best_k = overall_best_entry['k_Optimal']
final_labels = final_best_model.predict(X_scaled)

print("\n" + "="*60)
print(f"🏆 KEPUTUSAN AKHIR: MODEL {overall_best_entry['Skenario']} DIPILIH")
print("="*60)
print(f"Jumlah Profil (k) : {final_best_k}")
print(f"BIC Terendah       : {overall_best_entry['BIC']}")
print(f"Entropy           : {overall_best_entry['Entropy']}")
print(f"Min Class Prop    : {overall_best_entry['Min_Prop']}% (Threshold > 5%)")

print("\n💡 Catatan: Skenario otomatis menghasilkan k={best_k_3}. ")
if final_best_k == best_k_3:
    print("✅ Hasil Konvergen! Pendekatan Bayesian mendukung pemilihan jumlah klaster ini.")
else:
    print("⚠️ Terdapat perbedaan tipis antara metode statistik (BIC) dan probabilistik (Bayesian).")
    print(f"   Model k={final_best_k} tetap dipilih karena lebih parsimonius berdasarkan kriteria BIC.")
...
# CHANGED: Gunakan final_labels dari hasil seleksi model terbaik, bukan best_labels dari Skenario 1
df_final['Klaster'] = final_labels
...
# ==============================================================================
# 5.3 ANALISIS KONTRIBUSI PARAMETER / FEATURE IMPORTANCE
# ==============================================================================
from sklearn.ensemble import RandomForestClassifier

# Pastikan menggunakan label dari model terbaik yang sudah dipilih.
# CHANGED: final_labels sudah ditentukan dari seleksi model terbaik (line 984),
# sehingga tidak perlu di-assign ulang dengan best_labels
# final_labels sudah berisi label dari overall_best_model.predict(X_scaled)
df_final['Klaster'] = final_labels

# ------------------------------------------------------------------------------
# BAGIAN A: GLOBAL FEATURE IMPORTANCE (Menggunakan Random Forest)
# Tujuan: "Secara keseluruhan populasi, variabel apa yang paling kuat membelah klaster?"
# ------------------------------------------------------------------------------
# Latih Random Forest sebagai Post-Hoc Classifier
rf = RandomForestClassifier(n_estimators=100, random_state=42)
rf.fit(X_scaled, final_labels)

# Ekstraksi Feature Importance
global_importance = pd.DataFrame({
    'Parameter (Broad Scales)': SELECTED_SCALE_COLS,
    'Global Importance (%)': rf.feature_importances_ * 100
}).sort_values(by='Global Importance (%)', ascending=False)

print("\n--- A. Global Feature Importance (Daya Pembeda Keseluruhan) ---")
display(global_importance.round(2))

# Visualisasi Bar Plot
plt.figure(figsize=(10, 5))
sns.barplot(x='Global Importance (%)', y='Parameter (Broad Scales)', data=global_importance, palette='viridis')
plt.title('Daya Pembeda Keseluruhan: Parameter Penentu Pembagian Profil Laten', fontweight='bold')
plt.xlabel('Tingkat Kepentingan / Pengaruh (%)')
plt.tight_layout()
plt.show()

# ------------------------------------------------------------------------------
# BAGIAN B: LOCAL FEATURE CONTRIBUTION (Berbasis Jarak Z-Score Centroid)
# Tujuan: "Khusus di klaster tertentu, variabel apa yang jadi ciri khas utamanya?"
# ------------------------------------------------------------------------------
# 1. Hitung Centroid (Rata-rata T-Score asli per profil)
centroids = df_final.groupby('Klaster')[SELECTED_SCALE_COLS].mean()

# 2. Transformasikan centroid ke skala Z-Score lalu ambil nilai absolutnya
# Nilai absolut Z-Score menunjukkan seberapa jauh fitur tersebut menyimpang dari rata-rata populasi
centroids_abs = np.abs(scaler.transform(centroids.values))

# 3. Hitung persentase kontribusi per baris (per klaster)
cluster_contributions = (centroids_abs / centroids_abs.sum(axis=1)[:, None]) * 100

# 4. Tampilkan Tabel Kontribusi per Klaster
...
print("\n--- B. Local Feature Contribution (Persentase Ciri Khas per Klaster) ---")
display(cont_df)

# 5. Visualisasi Heatmap Kontribusi Lokal
plt.figure(figsize=(12, 6))
sns.heatmap(cont_df.T, annot=True, fmt=".1f", cmap='YlGnBu', cbar_kws={'label': 'Kontribusi Lokal (%)'})
plt.title('Keunikan Lokal: Variabel Mana yang Paling Mendefinisikan Tiap Cluster?', fontweight='bold')
plt.ylabel('7 Broad Scales')
plt.xlabel('Profil Laten (Cluster)')
plt.tight_layout()
plt.show()

"""## 5.4 Interpretasi Substantif & Validasi
...
# CHANGED: Simpan model GMM final (hasil seleksi terbaik dari semua skenario)
# Sebelumnya menggunakan best_model (hanya Skenario 1), sekarang menggunakan final_best_model
joblib.dump(final_best_model, 'artifacts/gmm_final_model.pkl')
